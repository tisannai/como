...
typedef struct como_cmd_s {
  ...
  struct como_cmd_s* parent;

  /** Number of subcmds. */
  int subcmdcnt;

  /** Array of subcmds. */
  struct como_cmd_s** subcmds;
 ...
 * (including parsing).
 */
#define como_command( prog,author,year,...) do {                        \
 ...
 * (including parsing).
 */
#define como_complete( prog,author,year,...) do {                       \
    como_init( argc, argv, author, year );                              \
    como_subcmd( prog, NULL, __VA_ARGS__ );                             \
    como_finish();                                                      \
  } while ( 0 )
 ...
 */
#define como_maincmd( prog,author,year,...) do {                        \
    como_init( argc, argv, author, year );                              \
    como_subcmd( prog, NULL, __VA_ARGS__ );                             \
  } while ( 0 )
...
#define como_subcmd( name,parentname,...) \
  como_spec_subcmd( name,                                               \
                    parentname,                                         \
                    (como_opt_spec_t [])                                \
                    {  __VA_ARGS__ }, como_spec_size( __VA_ARGS__ ) )

/**
 * Option specification list (array) size.
 */
#define como_spec_size(...) (sizeof((como_opt_spec_t []){ __VA_ARGS__ })/sizeof(como_opt_spec_t))
